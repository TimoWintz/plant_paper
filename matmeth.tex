\tim{\paragraph{Target applicative setup.} TODO: speak about the
real scanner.  }{}
...
modelling. \tim{The model plant \textit{Arabidopsis Thaliana} was chosen to
...
\tim{A generative model of \textit{A. Thaliana} was specifically
written in the L-py programming language. L-py is a python extension,
part of OpenAlea, which implements L-systems.}{3D meshes of \textit{A.
...
\tim{Comment: I think if we want to explain L-system, we have to
provide a simple example with the associated rewriting rules}{}

\tim{Randomness can be added to the models by asigning different
...
for \textit{A. Thaliana} can be found online~\cite{}.}{}
...
\tim{Comment: move this down}{The API was used to simulate a virtual scanner and generate images
...
original scene lightning\tim{. In order to increase the complexity of the
...
The \tim{ground truth plant part segmentation}{virtual labels} are acquired by rendering the plant material by material, with one material per
\tim{plant part}{organ}~(Figure \ref{fig:plants}). The
\tim{plant parts}{organs} considered for \textit{A. Thaliana} are leaf, stem, flower,
...
The image segmentation was performed using segmentation convolutional
